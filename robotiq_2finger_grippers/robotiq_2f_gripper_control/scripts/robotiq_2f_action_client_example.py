...
def operate_gripper():
    ...
    # while not rospy.is_shutdown():
    Robotiq.goto(robotiq_client, pos=1.00, speed=0.1, force=1000 , block=True)
    rospy.sleep(1)
    robotiq_client.wait_for_result()
    # Robotiq.goto(robotiq_client, pos=0.00, speed=0.01, force=0.4 , block=True)
    Robotiq.close(robotiq_client, block=True)
    robotiq_client.wait_for_result()
