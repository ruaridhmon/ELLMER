...
def go_to_pose(base, target_pose, angular_speed=10, linear_speed=0.05, angular_direction="pitch"):
    ...
    while not_at_pose:
        velocity = go_to_pose_velocity(base.current_pose, target_pose, linear_speed, angular_speed, angular_direction)
        distance_linear = calculate_distance(base.current_position, target_pose[:3])
        if angular_direction:
            angular_difference = calculate_angular_displacement(base.current_pose[3:], target_pose[3:], angular_direction)
        else: 
        ...
        if distance_linear < 0.03 and abs(angular_difference) < 2:
            base.robot.publish_safe_twist([0,0,0,0,0,0])
            not_at_pose = False

        base.robot.publish_safe_twist(velocity)
        rospy.sleep(update_time)
...
def drawing(self, object="bird"):

    speed = 0.01
    update_time = 0.1

    rospy.sleep(0.4)
    prompt = f'"silhouette of a {object} against a white background. nothing else. no shadow"'
    waypoints = return_waypoints(prompt)
